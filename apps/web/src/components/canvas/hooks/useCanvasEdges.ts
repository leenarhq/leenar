        ...
        addEdge(
          {
            ...
            type: "blueprint",
            animated: isRunning,
            // No `color`: BlueprintEdge derives the arrowhead from the same
            // `var()` as its line, so a colour stored here would be a stale
            // copy of a value the renderer never reads.
